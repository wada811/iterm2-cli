...
- **`ITerm2Adapter`（port）**: `send_text` / `send_key` / `get_screen_contents` / `split_pane` / `create_tab` / `create_window` / `activate` / `close` / `get|set_variable` / `list_sessions` 等の最小インターフェース。中核ロジックはこれにのみ依存。
...
| （なし・`session.list` 合成）| — | `identify` | current をクライアント側で解決し（D5）`list` から SessionInfo を引く。専用 socket method を持たない |
...
| `session.wait` | session, until, until_text, timeout, poll_interval | `wait` | `wait_until`（既定 until=idle。`until_text` 指定時は画面に marker 出現を待つ）|
| `pane.split` | session, vertical, before, profile | `new-split` | `async_split_pane`（`before` で source の前に作る）→ 新 session_id。CLI は方向 right/left/down/up を vertical/before に写像 |
| `window.new_tab` | profile, command, window_id, from_session | `new-tab` | `get_window_by_id(wid).async_create_tab`（`window_id` 指定時）/ from_session を含む窓に `async_create_tab` → 新 session_id。窓が皆無のときだけ新規窓にフォールバック |
| `window.new` | profile, command | `new-window` | `Window.async_create` → 新規ウィンドウの session_id |
| `session.focus` | session | `focus` | `async_activate` |
| `session.set_name` | session, name | `rename` | `async_set_name`（ペイン表示名を変更）|
...
本 CLI は単体で完結し、特定の上位ツールを前提としない。外部の利用側は次の 3 経路のいずれでも統合できる:
...
利用側は次の安定インターフェースに依拠して組み込む（内部実装に依存しない）:

- **コマンド表面・exit code・`--json` の形**: [README](../README.md) を正とする。
- **socket プロトコル**: §3（`{id,method,params}` / `{id,ok,result|error}`、名前空間 method）。
- **公開ライブラリ API**: `iterm2_cli` の `Controller` / `RealAdapter` / `Backend` / `SessionResolver` / `State`。
- **状態報告の規約**: `set-status itermcli_state …`（または OSC 1337 SetUserVar で `user.itermcli_state`）を書けば `busy`/`wait` が拾う。ペイン識別は `label set <name> <id>`。

### ドメイン特化ラッパーが「可能」

特定ツール（例: AI コーディングエージェント）固有のお作法 — 起動コマンド、状態 hook の配線、TUI 固有の
送信シーケンス、命名/オーケストレーション方針 — は、上記プリミティブの上に**薄いラッパーとして外側に**
載せられる（本リポジトリには含めない）。本 CLI はそれを**可能にする**だけで、特定のラッパーを前提にしない。
...
- **結合テストの穴**: `wait`（実 busy→idle 遷移）は実機未被覆（非破壊で busy を再現しづらいため）。`set_name` / `create_tab`（既存窓・from_session）/ `create_window` は結合スモークで被覆済み。
