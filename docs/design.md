...
| `window.new_tab` | profile, command, new_window, window_id | `tab` | `async_create_tab` / `Window.async_create` / `get_window_by_id(wid).async_create_tab`（`window_id` 指定時）→ 新 session_id |
| `session.focus` | session | `focus` | `async_activate` |
| `session.set_name` | session, name | `set-name` | `async_set_name`（ペイン表示名を設定）|
