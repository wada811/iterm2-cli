...
def identify(
    ...
    """呼び出し元（current）の session を特定して出力する（cmux identify 相当）。

    current 解決はクライアント側で行う（D5）。`--json` で全フィールド＋割当 label を返す。
    """

    def run(c: Controller):
        info = c.identify(target, session=session)
        # session_id→label の逆引き（ローカル LabelStore。複数 label が同一 session を指しうる）。
        labels = sorted(k for k, v in LabelStore().all().items() if v == info.session_id)
        out = asdict(info)
        out["labels"] = labels
        _emit(
            out,
            json_out,
            lambda o: typer.echo(
                f"{o['session_id']}  {o['name']}"
                + (f"  [{', '.join(o['labels'])}]" if o["labels"] else "")
            ),
        )
